...
if generate_btn and prompt:
    ...
    try:
        ...
        for i in range(int(variants)):
            ...
            # print(f"Raw GPT output for variant {i}:", repr(output))
            try:
                # Try loading as a single JSON object or array
                if output.startswith('['):
                    arr = json.loads(output)
                    result = arr[i] if i < len(arr) else {}
                else:
                ...
                try:
                    ...
                    result = {
                        "body": "Sorry, error parsing GPT output.",
                        "placeholders": [],
                        "length": 0,
                        "variant_id": None
                    }
